...
На папку, в которую записываются JS-файлы ассетов, может потребоваться дать права, напр. сделать владельцем пользователя, 
под которым у вас работает php, например: 
```bash
